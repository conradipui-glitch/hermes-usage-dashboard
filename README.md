...
Дашборд собирает данные из `state.db` и всей ротации журнала (`logs/agent.log`, `agent.log.1`, `agent.log.2`, …), показывает токены, модели, провайдеров, задачи и live-ленту событий — без изменения файлов Hermes и без отправки данных в интернет.
